...
class ControllerV2:
    def __init__(self, thermocouple, heater, motor, storage, kp: float, ki: float, kd: float, sample_s: float, output_limits: Tuple[float, float], safety_cfg: dict, autotune_cfg: dict = None, on_update: Optional[Callable[[dict], None]] = None):
        self.tc=thermocouple; self.heater=heater; self.motor=motor; self.storage=storage; self.on_update=on_update
        self.pid=PID(kp,ki,kd,setpoint=25.0,sample_time=sample_s,output_limits=output_limits)
        self.enabled=False; self.sample_s=sample_s
        self._stop=threading.Event(); self._thread=threading.Thread(target=self._loop,daemon=True)
        self.safety={"absolute_max_c":float(safety_cfg.get("absolute_max_c",350.0)),"sensor_fault_action":safety_cfg.get("sensor_fault_action","cut"),"max_over_sp_c":float(safety_cfg.get("max_over_sp_c",40.0))}
        self._status={"pv":None,"sp":self.pid.setpoint,"u":0.0,"enabled":self.enabled,"motor":0.0,"alarm":None,"autotune":{"active":False}}
        self.autotune=None; self.autotune_cfg=autotune_cfg or {}
        if bool(self.autotune_cfg.get("enabled",False)): self._init_autotune()
    ...
    def stop(self, join_timeout: float = 2.0):
        self._stop.set();
        if self._thread.is_alive(): self._thread.join(timeout=join_timeout)
    def set_setpoint(self, temp_c: float):
        self.pid.setpoint=float(temp_c); self._status["sp"]=self.pid.setpoint; self.pid.reset();
        if self.autotune: self._init_autotune()
    def set_gains(self, kp: float, ki: float, kd: float):
        self.pid.kp,self.pid.ki,self.pid.kd=float(kp),float(ki),float(kd); self.pid.reset()
    def enable_control(self, enable: bool):
        self.enabled=bool(enable); self._status["enabled"]=self.enabled
        if not enable and self.heater is not None:
            try: self.heater.set_duty(0.0); self._status["u"]=0.0
            except Exception: pass
    def set_motor_speed(self, percent: float):
        self._status["motor"]=max(0.0,min(100.0,float(percent)))
        if self.motor is not None:
            try: self.motor.set_percent(self._status["motor"])
            except Exception: pass
    def apply_part(self, code: str):
        part=self.storage.get_part(code)
        if not part: return False,None
    ...
    def autotune_stop(self):
        self.autotune=None; self._status["autotune"]={"active":False}
        if self.heater is not None:
            try: self.heater.set_duty(0.0)
            except Exception: pass
    def autotune_apply(self):
        auto=self._status.get("autotune",{})
        if not auto.get("done"): return False,None
        kp,ki,kd=auto.get("Kp"),auto.get("Ki"),auto.get("Kd")
        if kp is None: return False,None
        self.set_gains(kp,ki,kd); return True,{"Kp":kp,"Ki":ki,"Kd":kd}
    def _loop(self):
        while not self._stop.is_set():
            now=time.monotonic(); pv=None; alarm=None
            try:
                if self.tc is not None: pv=self.tc.read_c()
            except Exception: pv=None
            self._status["pv"]=pv; cut=False
            if pv is None:
                if self.safety.get("sensor_fault_action")=="cut": cut=True; alarm="SENSOR_FAULT"
            else:
                if pv>=self.safety.get("absolute_max_c",350.0): cut=True; alarm="ABS_MAX"
                elif pv-self.pid.setpoint>self.safety.get("max_over_sp_c",40.0): cut=True; alarm="OVER_SP"
            ...
            if self.autotune and not cut and pv is not None:
                ...
                if self.enabled and not cut and pv is not None:
                    ...
                    self.pid.reset(); u=0.0
            self._status["u"]=u; self._status["alarm"]=alarm; self._status["autotune"]=auto_status
            if self.on_update:
                try: self.on_update(dict(self._status))
                except Exception: pass
